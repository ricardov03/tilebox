...
- Icons: two sets, `line-md` (the default) and `simple-icons` (brands). The icon search in `/edit` reads the two installed packs from `node_modules/`, so it works offline and no search text leaves your machine. `line-md` comes first, and brands that simple-icons removed (LinkedIn, Twitter, Amazon, Slack) are filtered out.
