...
- Icons: two sets, `line-md` (the default) and `simple-icons` (brands). The icon search in `/edit` AND the little pictures it shows both read the two installed packs from `node_modules/`, so the editor works offline, no search text leaves your machine and no icon is fetched from the internet. `line-md` comes first, and brands that simple-icons removed (LinkedIn, Twitter, Amazon, Slack) are filtered out.
...
An older `profile.json` without `email`, `showEmail` and `highlights` is upgraded by `npm run dev` (`ensure:profile`): it adds `"email": "you@example.com"`, `"showEmail": false`, `"highlights": []` and changes nothing else. `check:profile` warns while the placeholder email is still there. A file that has some of those keys and cleared its email keeps no address: only a file with none of the three gets the placeholder (WP20).
...
Icons come from **two sets only**: `line-md` (the default) and `simple-icons` (brands that `line-md` lacks). So a name always starts with `line-md:` or `simple-icons:`, like `line-md:github`. Any other prefix is refused by the schema, by the editor's save and by `npm run check:icons`, with the message "Use an icon from line-md or simple-icons". Browse https://icones.js.org/collection/line-md and https://icones.js.org/collection/simple-icons.
Icons that simple-icons marks as hidden (LinkedIn, Twitter, Amazon, Slack: removed brands) are refused too. `line-md:linkedin` works.
...
| `npm run ensure:profile` | Creates `content/profile.json` from the example when it is missing, adds keys a newer version needs, and removes a block `icon` from an icon set tilebox no longer supports (one line per change) |
| `npm run check:profile` | Validates the profile and prints which file is used (personal or example) |
| `npm run check:icons` | Fails on an icon that is not from `line-md` or `simple-icons`, does not exist in its pack, or is marked hidden: your profile, the social map, the UI icons and the brand map |
...
| Icons | `@nuxt/icon` with the two local Iconify packs (`line-md`, `simple-icons`), the only sets tilebox allows (`app/utils/icon-sets.ts`). Bundled at build, no runtime API. The editor's search and icon previews read those packs from disk (`content/icon-index.ts`), never `api.iconify.design` |
...
server/api/              development-only routes: profile, save, upload, icons/{search,svg} (local, no network), avatar/gravatar, unfurl, site/assets, site/upload, site/qr.png, links/check, images/pexels/{status,search,pick}
...
| My icon was removed at startup | `npm run dev` printed `profile: removed icon "<name>" from block <id> (only line-md and simple-icons are supported)`. Tilebox now takes icons from `line-md` and `simple-icons` only, so a name from another set was cut out of that block and the tile fell back to its automatic icon. Pick a new one in `/edit` > the block > Icon. Your file was rewritten once; nothing else changed |
| `npm run check:icons` fails | The name is not from `line-md` or `simple-icons`, the icon does not exist in that pack, or the pack marks it hidden (a removed brand). Pick one at https://icones.js.org/collection/line-md or https://icones.js.org/collection/simple-icons |
| The icon search finds nothing | Type fewer letters: the search matches icon names only (`mail`, `git`, `you tube`). It never asks the internet, so a name that is in neither installed pack cannot be found |
