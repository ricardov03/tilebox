...
test('a cleared alt field is not the owner\'s own text: the pick fills it, and the soft hint goes (WP17: no "Required")', async ({ page }) => {
  const id = imageBlockId()
  await mockPexels(page)
  await openImageForm(page)
  const altField = page.locator('form input[id$="-alt"]')
  ...
  // The owner writes an alt, then clears the field. WP17: no error, the draft loses the alt, a soft hint shows.
  await altField.fill('My own words for this photo')
  await altField.blur()
  await altField.fill('')
  await altField.blur()
  await expect(page.locator('form [data-field-error]')).toHaveCount(0)
  await expect(altField).not.toHaveAttribute('aria-invalid', 'true')
  await expect(page.locator('form [data-alt-hint]')).toContainText('Add a description for screen readers')
  ...
  await page.getByLabel('Search Pexels').press('Enter')
  await page.locator('button[data-photo-id="333333"]').click()
  // The stock alt goes in and the hint is gone.
  await expect(tile.locator('img')).toHaveAttribute('alt', 'A desk, photo 333333')
  await expect(altField).toHaveValue('A desk, photo 333333')
  await expect(page.locator('form [data-alt-hint]')).toHaveCount(0)
  await expect(page.locator('form [data-field-error]')).toHaveCount(0)
  await expect(page.locator('[data-field-problems]')).toHaveCount(0)

  // Cleared again, then the SAME photo: the field gets the alt back.
  await altField.fill('')
  await altField.blur()
  await expect(page.locator('form [data-field-error]')).toHaveCount(0)
  await page.locator('button[data-photo-id="333333"]').click()
